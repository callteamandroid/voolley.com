            ...
            <div class="modal-body">
                <div class="input-group mb-3">
                    <div class="input-group">
                        ...
                        // select proveedores
                        $sql1 = $conexion->query("SELECT * FROM orden WHERE status = 'ABIERTA' ORDER BY noOrden");
                        $oc = ["Selecciona Orden de Compra"];
                        while ($resultado1 = $sql1->fetch_assoc()) {
                            $idOc = $resultado1['id'];
                            $noOc = $resultado1['noOrden'];
                            $oc[$idOc] = $noOc;     
                        }
...
function guardarCompra(event) {
    ...
    xhr.open('POST', './compras/guardar_compra.php', true);
...
document.getElementById('provedori').addEventListener('change', function() {
    ...
    console.log(orden);
    if (proveedor) {
        fetch('./compras/obtener.php?proveedor=' + proveedor)
            .then(response => response.json())
            .then(data => {
                productosOrden = data; // Aquí se actualiza productosOrden
                console.log(data);
                const productosOC = document.getElementById('productosOC');
                const tbody = productosOC.querySelector('tbody');
                ...
                if (Array.isArray(data)) {
                    data.forEach(producto => {
                        ...
                        const cantidadMaxima = producto.cantidad; // Usamos el stock disponible del producto

                        // Crear el contenido HTML para la fila de la tabla
                        row.innerHTML = ` 
                            <td>${producto.noParte}</td>
                            <td>${producto.descripcion}</td>
                            <td id="cantidadDisponible_${producto.noParte}" data-max="${producto.cantidad}">${producto.cantidad}</td>
                            <td>
                                <select class='form-select' id='ubicacion_${producto.noParte}'>
                                    <option selected>Seleccione una ubicación</option>
                                    <optgroup label='Zonas'>
                                        <option value='zona_refrigeracion'>Zona de Refrigeración</option>
                                        <option value='zona_entrada'>Zona de Entrada</option>
                                        <option value='zona_salida'>Zona de Salida</option>
                                        <option value='zona_almacen_general'>Zona de Almacén General</option>
                                        <option value='zona_embalaje'>Zona de Embalaje</option>
                                        <option value='zona_picking'>Zona de Picking</option>
                                    </optgroup>
                                </select>
                            </td>
                            <td><input type="number" class="form-control" value="1" min="1" max="${cantidadMaxima}" id="cantidad_${producto.noParte}" data-max="${producto.cantidad}" data-original="${producto.cantidad}"></td>
                            <td><input type="text" class="form-control" value="${producto.precio}" disabled></td>
                            <td>
                                <button id="${producto.noParte}" class="btn btn-success agregar" 
                                    data-codigo="${producto.noParte}" 
                                    data-nombre="${producto.descripcion}"
                                    data-costo="${producto.precio}">
                        ...
                        const cantidadInput = row.querySelector(`#cantidad_${producto.noParte}`);
                        const agregarBtn = row.querySelector('.agregar');

                        // Evento para el botón de agregar
                        agregarBtn.addEventListener('click', function() {
                            const cantidad = parseInt(document.getElementById(`cantidad_${producto.noParte}`).value);
                            const ubicacion = document.getElementById(`ubicacion_${producto.noParte}`).value;
                            document.getElementById("provedori").disabled = true;

                            // Validar si se ha seleccionado una ubicación
                            if (ubicacion === "Seleccione una ubicación" || ubicacion === "") {
                                alert('Por favor, selecciona una ubicación para el producto');
                                return;
                            }

                            // Obtener el stock original y el stock disponible (restante)
                            const cantidadOriginal = parseInt(document.getElementById(`cantidadDisponible_${producto.noParte}`).getAttribute('data-max'));
                            const cantidadSeleccionada = productosSeleccionados.filter(p => p.codigo === producto.noParte)
                                                .reduce((total, p) => total + p.cantidad, 0); // Total de la cantidad seleccionada
                            const cantidadRestante = cantidadOriginal - cantidadSeleccionada;

                            // Validar que la cantidad no sea mayor que el stock restante
                            if (cantidad > cantidadRestante) {
                                alert(`La cantidad ingresada (${cantidad}) no puede ser mayor que el stock restante (${cantidadRestante}) para el producto ${producto.descripcion}`);
                                return;
                            }
                            ...
                            const productoSeleccionado = {
                                codigo: producto.noParte,
                                nombre: producto.descripcion,
                                cantidad: cantidad,
                                costo: parseFloat(producto.precio),
                                ubicacion: ubicacion
                            };

                            productosSeleccionados.push(productoSeleccionado);

                            // Actualizar la cantidad disponible en productosOrden
                            let productoOrden = productosOrden.find(p => p.noParte === producto.noParte);
                            if (productoOrden) {
                                productoOrden.cantidad -= cantidad;
                            }

                            // Actualizar la tabla de productos seleccionados
                            actualizarTablaSeleccionados();

                            // Restar la cantidad seleccionada de la cantidad disponible en la tabla
                            let cantidadDisponible = parseInt(document.getElementById(`cantidadDisponible_${producto.noParte}`).textContent);
                            cantidadDisponible -= cantidad;
                            document.getElementById(`cantidadDisponible_${producto.noParte}`).textContent = cantidadDisponible;

                            // Deshabilitar el botón si la cantidad seleccionada alcanza el stock original
                            if (cantidadDisponible <= 0) {
                                agregarBtn.disabled = true; // Deshabilitar el botón de agregar
                            }

                            // Comprobar si todas las cantidades de productos en productosOrden son 0 y cambiar el estado
                            actualizarEstado();
                        });
                    });
                } else {
                    console.error('El formato de los productos no es un array:', data);
                }
            })
            .catch(error => {
...
function actualizarTablaSeleccionados() {
    ...
    productosSeleccionados.forEach(function(producto, index) {
        ...
        // Mostrar la cantidad como un valor estático en lugar del campo de input
        fila.insertCell(2).textContent = producto.cantidad; // Solo texto, no input

        fila.insertCell(3).textContent = producto.costo.toFixed(2);
        fila.insertCell(4).textContent = (producto.cantidad * producto.costo).toFixed(2);
        fila.insertCell(5).textContent = producto.ubicacion; // Mostrar la ubicación en la tabla
